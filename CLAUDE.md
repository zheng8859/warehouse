...
剩余：阶段六的 KPI 看板（`18` 号 · 关键路径 F10，独立 change）+ 阶段八（`32`/`33` · v1.0.0）。**
...
- KPI 看板（`18` 号 · 关键路径 F10）：看板四指标的**实时聚合**已落地并提交（`feat(eval)`，
  `GET /api/kpi/summary` —— 同物料跨巷道均值 / 拣货量加权集中度 / 单张开单跨巷道均值 /
  推荐采纳率，全部从真实数据计算，P6 看板卡片已接）。剩余 `KpiSnapshot` 按周/月落库 +
  环比聚合的独立 change（本次评测 change 的 L3 只测其指标纯函数计算与阈值判定，未实现聚合落库）
