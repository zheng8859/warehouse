...
**阶段一~八已收尾并打标（`v0.1.0` ~ `v0.7.0` → `v1.0.0`）。全部工作闭环、项目结项 ——
...
「实体字段以 `17` 为准」这条口径持续失真。该变更**已归档**（2026-09-12）
...
当前 **787 passed**（本阶段多次实测 4.8–6.6s，**横跨 `00` 的 `<5s` 预算线**，
钩子不强制，见记忆条目 `l1-budget-pressure`），其中逻辑层 493 条 / 约 1.5s。
变更 `recommendation-engine` 的任务 **40/40 完成**（全部实现并变异验证），**已提交**
—— **已合并（`944b996`）、已打 `v0.3.0`、已归档（2026-09-12）**。`openspec validate`
退出 0（14 条 `⚠` 全是 RFC2119 假警报，见记忆条目）。
...
**阶段五（`29` · 设计 `10` · 冷路径 LLM · 已收尾 `v0.5.0`）**：`app/llm/` 冷路径子系统 ——
出境脱敏管线（正向白名单 10 字段）、外部 LLM 客户端（provider 抽象 + 超时 + mock）、
冷路径网关编排（统一链路 + 双产物响应）、成本三道护栏 + 同步记账（token / 并发 / 月预算）；
四类能力 ① KPI 解读 ② 偏离归因 ③ 权重调优建议（≥50 批次反事实模拟 + 影子模式 PROPOSED）
④ 移库多方案（规则算三档 + 量化代价 + 三重校验）+ 采纳落地 `weight/apply`（规则校验写新版
`WeightConfig` + 版本化）；对话台统一入口 `POST /api/conversation/message`（L0/L2 + 写意图零台账）+
意图路由表（4 类意图 → 能力/槽位/write_intent）；冷路径 6 端点 `POST /api/llm/*`；
AI Notice 强制注入（`field_serializer` 挂所有 ai 叙事）；`ai.*` 权限四标识（17→21）+ 端点级鉴权
（`require_cold_path_enabled` 开关守卫 + `require_permission`）；冷路径成本护栏配置 + 3 实体 + 迁移。
**1375 passed**（全量，零回归）。变更 `ai-assist` 已归档（2026-09-14），主规格
...
**阶段六（`30` · 设计 `18`+`20` · 评测体系 · 已收尾 `v0.6.0`）**：`backend/evals/` 三层评测框架 ——
Golden 数据集 60 场景（`golden_001~060`，基线 24 / 边界 20 / 回归 16，13 维度，可溯源 20号
SC/CL/AC/TR）；`eval_utils.py` 5 纯函数门面（收编 `services/kpi.py` / `engine/degradation.py` /
`engine/scoring.py` / `llm/redact.py` 口径，同源不漂移）；`run_evals.py` CLI（`--tier`/`--dimension`/
`--compare`/`--save-baseline`/`--run-slow`/`--json`，劣化分流 ≤3% 记录 / 3–5% 告警 / >5% 阻断 +
P0 护栏「台账完整性 / 决策可追溯 / 出域=0」立即阻断）；L1 单元 31 / L2 集成 32 / L3 质量 17 +
冷路径观测 1（冷路径 LLM「建议可用性」非闸门）；`baseline.json` 基线（拒绝下调）+ pre-push
`evals-baseline` 钩子 + `.github/workflows/evals-ci.yml` 参考物。**1473 passed**（全量，零回归）。
变更 `evals` 已归档（2026-09-15），主规格 `openspec/specs/evals/spec.md`（10 需求）已由 delta
写入。**已合并（`0f7b131`）、已打 `v0.6.0`。**

**KPI 看板（`18` 号 · 关键路径 F10）已收尾**：看板四指标**实时聚合**已落地并提交（`feat(eval)`，
`GET /api/kpi/summary` —— 同物料跨巷道均值 / 拣货量加权集中度 / 单张开单跨巷道均值 / 推荐采纳率，
全部从真实数据计算，P6 看板卡片已接）。`KpiSnapshot` 按周/月落库 + 环比聚合**明确不做**
...
- 灰度部署（`33` Step 2/3 `/land-and-deploy` + `/canary`）：单厂试点、0 实时接口、本地 SQLite、
  无真实部署目标 —— 不做灰度上线；将来如需上线，按 `19` §3 独立部署形态执行
- 运行时一键 toggle 端点：`19` §4.5 预案是「关」而非「频繁切」，配置项开关已够用；
  真要做需新增管理员专属端点与权限设计，另开 change
