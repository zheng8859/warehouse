...
- [x] 7.4 `/api/auth/login` 端点（免认证、返回凭据、首次登录标记须改密）；验证：`tests/api/test_auth.py` 断言未携带凭据时返回 200 与凭据
      （已完成。端点落 `app/api/routes/auth.py`，报文 DTO 落 `app/schemas/auth.py`（**不是** 17 §十 那 6 类结构：
      那 6 类是存进实体列的领域载荷，本文件只在 HTTP 边界上活）。三处开发阶段决策：报文用 JSON（前端是零构建
      Vanilla fetch 的同源请求）；字段名就是 `username`/`password`（不套 `OAuth2PasswordRequestForm` —— 它带着
      `grant_type`/`scope` 等本产品不用的字段，引进来只会让人以为这里有 OAuth）；**不给 `password` 设长度上限**
      （72 字节是 bcrypt 的实现事实而非产品策略，写成报文校验会让超长口令得到 422 而其它失败得到 401，
      同一件事出现两种失败形状）。成功响应带 `user_id`/`role`/`warehouse_id`（13 §8.1 的会话级存储）、
      `expires_in`、`must_change_password`；**只给标记不做拦截**，见 9.4g①；凭据只走响应体**不下发 Cookie**
      （13 §7.3/§8.1：客户端持久化 + 拦截器拼头；两处都写会让「登出要清哪个」变成真问题），
      故本阶段**不实现登出端点**。成功时写 `last_login_at`（13 §5.2），失败不写 ——「最后登录时间」不是
      「最后尝试时间」，混用会让审计读错）
- [x] 7.5 登录失败语义：用户名不存在与口令错误**返回一致**的 401，不泄露账号是否存在；验证：同文件断言两次响应状态与响应体一致
      （已完成，且比任务要求更严：**五种**失败形态（不存在 / 口令错 / 未激活 / 已停用 / 已驳回）的响应
      逐字一致 —— 状态本身是敏感信息（离职、调岗、驳回），而 22 §2.1 的失败提示也只有一条文案。
      两处容易被漏掉的补齐：**判定顺序为先口令后状态**（反过来会在 bcrypt 之前返回，于是不需要正确口令、
      仅凭响应快慢就能问出「用户名存在且被停用」）；**用户名不存在时照样对着一份常驻填充哈希做一次校验**
      （否则「不存在」立刻返回，而 bcrypt 一次几十毫秒 —— 响应体一致只挡住读，挡不住计时；填充哈希惰性生成，
      成本因子与真实哈希同源）。为此把 `register_exception_handlers` 改成 `detail` 为 None 时**不出现该键**：
      401 有两处产生方（中间件与登录端点），形状必须同形，否则前端拦截器要写两个分支）
- [x] 7.6 中间件校验凭据中的 `status`，使 `disabled` 账号的旧凭据在下一次请求即失效（无需重启、无服务端黑名单）；验证：同文件断言置 `disabled` 后旧凭据返回 401
      （已完成：中间件在解码凭据之后**回查账号当前状态**，非 `active` 一律 401（`deps.assert_account_usable`，
      判定写成正向的 `is not ACTIVE` 而非「拦住 disabled」—— 后者会放过 `pending`/`rejected`）。
      会话短命、取完即关，不复用端点的会话。配套三例：停用即失效、**重新启用后旧凭据又能用**
      （13 §8.3 无黑名单的对称语义 —— 把这条钉成断言，免得后来者"顺手"加一张吊销表）、
      凭据指向不存在的账号 → 401。另有对照例证拦截不依赖进程重启或缓存失效。
      新增 `app.state.session_factory` 作为**唯一**的会话工厂读取点（中间件与 `deps.get_db` 共用，
      一次覆盖两处；否则测试换库漏掉一处会表现成「端点读测试库、中间件读开发库」的怪 401））
- [x] 7.7 覆盖 `26` 完成标准 #4 的四场景：白名单放行、有效凭据通过、过期凭据 401、权限拒绝路径；验证：同文件四例全绿
      （已完成，四例见 `tests/api/test_auth.py` 的 `test_standard_4_scenario_*`。第四例按 D9 处置：
      端点级资源鉴权不在本阶段，故在**新建的应用实例**上挂一条临时探针路由直接抛 `PermissionDenied`，
      验的是异常到响应的映射（403 + `{"error":"permission_denied","message","detail"}`）；
      另加一条对照 —— 同一条探针**无凭据**时是 401 而非 403（13 §六 的第 1、2 层有先后：
      反过来的话未登录请求能从 403 里读出「这个端点存在」。探针挂在新建实例上也避免污染
      `test_smoke.py` 的装配完整性断言）。本文件另含夹具自检 —— 断言 `app.state.session_factory`
      真的被换掉，防「覆盖没生效、中间件去读了真实开发库）

## 8. 权限矩阵补测（矩阵本身不改）

- [x] 8.1 4 角色 × 10 资源全组合断言，逐条对照 `13` §2.2；验证：`tests/logic/test_permissions.py` 覆盖全组合且与矩阵逐条一致（`26` 完成标准 #3）。
      新建 `tests/logic/test_permissions.py`（106 例），矩阵按 `13` §2.2 的**行**逐格抄成
      `MATRIX: tuple[tuple[Permission, bool, bool, bool, bool], ...]`（列序即文档的仓管员/计划员/主管/管理员），
      每行末尾括注文档原样取值，便于逐条回对。**不用「角色 → 集合」的摘要形式**：那种写法下把一格
      抄错只是多一个集合元素，肉眼审不出来。断言的层次是四层：①17 行与 `Permission` 的**集合**相等
      （行数相等挡不住「一重复一遗漏」）、②资源前缀恰为 10 个、③68 格逐格 `check()` 相等、
      ④`ROLE_PERMISSIONS` 与矩阵列**互为反演**（③④同时过才能排除「`check` 另有一套逻辑而数据已漂移」）。
      另加「四角色都有键」（缺一个会让 `check` 静默全 False）与「同样输入两遍同值」（红线）。
- [x] 8.2 边界用例：`supervisor` 无 `inbound.view`；`supervisor` 有 `outbound.view` 无 `outbound.operate`（以 `13` §2.2 为准，**不是** `26` 附录B）；`planner` 三作业 `operate` 全无；验证：同文件四例。
      多写了两条同类边界：计划员的 `data_import.view`（🔍=**看得到**，在 `check` 这一维上是 `True`
      —— 把它读成 ❌ 会让计划员看不到导入结果，而那正是它协调排产的输入）；以及
      `account.manage` **单角色**（仅 admin，它是 spec `auth`「非管理员被拒」判据的来源）。
      另加「管理员的权限是其余三角色的超集」（结构性质断言：将来给某角色加权限却忘了管理员时会红）。
- [x] 8.3 `AUTO_ONLY` 封闭性：`ledger.write` 与 `engine.invoke` 对四角色**恒为 False**；验证：同文件断言四角色两项均不可用（`26` 完成标准 #5）。
      两处断言：逐角色 `check()`（挡不住「在 `check` 里为这两值开特例分支」）+ `ROLE_PERMISSIONS`
      的**数据**里也不许出现（前端菜单过滤与将来的 RBAC 若绕开 `check` 直读集合，这里是最后一道）。
      `check()` 里 AUTO_ONLY 判定**先于**查表，正是为了让它在这种情形下仍然不放行。
- [x] 8.4 配置子模块受限：`supervisor` 仅 `weight`，其余 4 个子模块拒绝；`warehouse_keeper` 与 `planner` 无 `config` 权限；验证：同文件覆盖 5 个子模块 × 4 角色。
      `13` §2.1 与 `22` §2.1 ⑦ 只给了 5 个子模块的**中文名**，标识符留给实现，本阶段在
      `app/api/permissions.py` 增 `CONFIG_SCOPES`（`import_template` / `cap_caliber` / `weight` /
      `reserve_ratio` / `prompt`，顺序即文档 ①~⑤），`weight` 沿用既有字面量。
      它是**跨阶段契约**（阶段七按它渲染子导航胶囊），已登记 9.4h。断言：5 个子模块逐个 × 4 角色；
      未知子模块名对三角色拒绝、**管理员仍通过**（管理员的检查是无条件 True 而非「查范围表」——
      否则将来新增第六个子模块时管理员会在它上线那一刻失去它）。
      另记：`22` §2.1 ⑦ 说配置页「管理员专用」，与主管的 🔧 不冲突（前者指页面默认归属，后者指页内被限制到的一个子模块），
      `13` §3.1 的菜单表逐字写着 `配置页 | — | — | 🔧 权重 | ✅`，故以 `13` 为准，无需新登记。
- [x] 8.5 白名单恰好 4 条，且 `/api/*` 未认证返回 401 而非 404；验证：`tests/api/test_smoke.py` 的既有断言保持全绿，并新增未知 `/api/*` 路径的 401 断言。
      既有两条（白名单常量逐条相等、单条随机未知路径 401）保持全绿；新增 7 条参数化的
      未知路径 —— 换成**探测者真会试的形状**（白名单路径的子路径 `/api/auth/login/extra` 与父路径
      `/api/auth`、将来才有的 `/api/kpi/dashboard`、红线路径 `/api/ledger/write`、`/api` 与 `/api/`），
      并把 `/api/definitely-not-a-route` 的 401 与**真实存在但需认证**的 `/api/health` 的 401 比到
      **整个响应体相同**（只比状态码挡不住「两者都 401 但 `error` 不同」这类实现 —— 那样照样能逐条问出哪些路由存在）。
      原有「刻意不碰数据库」的立场未变：这些请求都在中间件的凭据解析处就返回，不会走到库里。
- [x] 8.6（新增，不在原任务清单内）**`check` / `check_config_scope` 的类型守卫** —— `Role` 与 `Permission`
      都是 `str` 枚举，裸字符串入参（`check("admin", "kpi.view")`）会因 `Enum.__hash__` 取**成员名**
      而查不中，**静默返回 False**：fail-closed 方向没错，但现场表现是「管理员登录后菜单全空」而日志无错。
      两个函数加 `isinstance` 守卫（与 `app/core/state_machine.py` / `account_state.py` 同一处置），
      裸字符串一律 `TypeError`。变异自检：摘掉两处守卫调用后，恰好
      `test_non_enum_arguments_are_rejected_loudly` 与 `test_scope_check_rejects_bare_strings`
      两条变红，其余 104 条不动 —— 守卫确实是这两条的判据。**矩阵本身未改**，故不违反 §8 标题的约束。

## 9. 验收与收尾

- [x] 9.1 端到端建库验收：空库 → `alembic upgrade head` → `autogenerate` 产生**空 diff**；验证：两步均退出码 0 且 diff 为空。
      实测（`WMS_DATABASE_URL` 指向一个**全新**临时目录下的空库，目录内文件数确为 0）：
      ① `alembic upgrade head` 依次跑完 6 条迁移（baseline → 主数据链 → 衔接链 → 作业链 →
      度量与身份 → 配置与对话），退出码 0；② 库内 **24 张表** —— 23 个实体 + `alembic_version`，
      `version_num = 62cdb8b54011`（head）；③ `alembic check` → 「No new upgrade operations detected.」；
      ④ 另跑一次 `alembic revision --autogenerate` 并把生成物**逐字读出来**：`upgrade()` 与
      `downgrade()` 两段各只有一个 `pass`，无任何 `op.*` 语句 —— 空 diff 是**看过**的，不是只看退出码。
      验证用的临时库与那份 autogenerate 产物**均已删除**（`git status` 空）。
- [x] 9.2 全量测试：`python -m pytest backend/tests --tb=short -q`；验证：全绿且 L1 单元 < 5 秒（pre-commit 门禁）。
      实测 **514 passed**：`tests/models` 196 · `tests/logic` 266 · `tests/api` 47 · `tests/test_infra.py` 5。
      耗时连续 5 次为 4.39 / 4.44 / 4.24 / 4.18 / 4.28 秒（中位 ~4.28s），**在 5 秒门禁内但余量只剩约 14%**；
      期间另见过一次 6.17s 的离群（同一份代码连跑五次，机器噪声）。预算构成与风险已登记 **9.4j**。
      `alembic check` 仍为空 diff。
- [x] 9.3 逐条核对 `26` 完成标准的 9 条，并对 **#9（`run_evals --tier l1`）出具书面豁免记录**；验证：核对清单落进本文件，9 条各有「达成」或「豁免 + 理由」：
      | # | 标准 | 结论 |
      |---|------|------|
      | 1 | 全部模型 pytest 通过 | **达成** —— 514 passed（见 9.2 的分组计数） |
      | 2 | JWT 签名/验证/过期/篡改 | **达成** —— `tests/logic/test_token.py` 16 例覆盖四场景，另加算法混淆两类（`alg=none` 三种形态、HS512）、缺 claim 须指名、篡改头/载荷/换密钥，以及「不引入 PyJWT / jose / authlib」的 AST 断言（D7） |
      | 3 | 全角色 × 资源权限断言 | **达成** —— §8.1：`tests/logic/test_permissions.py` 106 例，68 格逐格对照 13 §2.2 |
      | 4 | HTTP 中间件认证 + 鉴权四场景 | **达成（附一处实质说明）** —— `tests/api/test_auth.py` 的 `test_standard_4_scenario_*` 四场景齐备（白名单 / 有效凭据 / 过期凭据 401 / 权限拒绝 403 + 同探针无凭据时 401 的对照）。但 403 那一条是**探针端点**上的「异常 → 响应」映射，**不是**真实端点的资源级鉴权 —— D9 把端点级鉴权排在本阶段之外。故该标准的字面成立，实质覆盖的是第 2 层的错误映射；RBAC 强制本身仍是路线图（M5） |
      | 5 | `ledger.write` / `engine.invoke` 无手动入口 | **达成** —— 代码审查：全仓库 17 个路由中无任何一条暴露这两个动作（`grep` 路由表 + §8.5 新断言「`/api/ledger/write` 未认证 401 且该路由不存在」）；另有两处自动断言守着（`AUTO_ONLY` 封闭性 + `ROLE_PERMISSIONS` 数据里不得出现） |
      | 6 | Alembic 迁移可执行 | **达成** —— 9.1 |
      | 7 | `git tag v0.2.0` 存在 | **本阶段末完成**（9.5）—— 标签只能产生在合并之后，此刻 `git tag` 里没有它是**预期**，不是遗留 |
      | 8 | OpenSpec 归档完成 | **待办（合并后）** —— `openspec/specs/` 目前为空；本 change 仍在 `openspec/changes/data-model-permission`。**注意顺序**：`26` 的 Step 7 把归档排在 Step 8（合并 + 打标）**之前**，而 CLAUDE.md §6.1 排在其后（第 6 步合并、第 7 步归档）。两者会让归档提交落在不同分支上（分支内 vs 直接落 main）。本阶段按 CLAUDE.md（项目指令文件）执行，即合并后再 `/opsx:archive`，并把该措辞差记在 9.4j 一并待决 |
      | 9 | `run_evals --tier l1` 通过 | **豁免 + 理由（见下）** |
      **#9 的书面豁免**：`evals/run_evals.py` 是**刻意**以退出码 3 失败的骨架（`00` §3.2 的「钩子跑了但什么都没检查」防静默通过），真实现在阶段六（文档 30）；`20` 号的场景集与 golden 样本此刻并不存在，所以本阶段任何「通过」都只可能是空跑。`.pre-commit-config.yaml` 里 pre-push 的 `evals-baseline` 钩子处于**注释状态**（第 59~68 行），故它也不在门禁链上。豁免成立的前提是「它以设计的方式失败而非崩溃」—— 核对本条时发现它**连入口都失修**（裸 `%` 让 argparse 在构造期就抛错，退出码 1 + traceback，`--list` 同样不可用），已修并加 7 例契约测试钉住，详见 **9.4i**。**结论：豁免成立**，且成立条件是修好之后才满足的。
...
- [ ] 9.4g **§7 暴露的规格与任务差口**（spec `auth` 有要求、tasks §7 未派任务 —— 按「设计文档是事实来源」
      不擅自补设计，也不静默丢掉；**阶段收尾前须逐条给出「补实现」或「改 spec」的决定**）：
      ① **「改密前不得访问其他业务端点」的拦截未实现** —— spec `auth` 场景「首次登录强制改密」的后半句
      （GIVEN 已激活且初始密码尚未修改，WHEN 以初始密码登录，THEN 登录被接受，**但改密前不得访问其他业务端点**）。
      tasks §7 只派了「返回首次登录标记」。本阶段**只给标记不拦截**：中间件已把 `must_change_password`
      放进 `request.state`（拦截点就位），改密端点尚未存在。**待决**：本阶段补实现（则须同时补
      `/api/auth/change-password` 与白名单/拦截规则，后者会与 13 §6.1 的四条白名单冲突 —— 需先改 13），
      或把该场景拆到路线图（则 spec 归档前须删掉这半句，否则归档后的 spec 会声称一个不存在的护栏）；
      ② **登出端点未实现**（13 §8.2 / spec「紧急吊销」的「登出不产生服务端记录」）—— 本阶段按无状态设计
      **刻意不实现**：13 §8.2 说登出只清客户端存储，服务端无记录可留，故端点存在与否不影响该场景成立。
      **待决**：若前端阶段（31）需要一个明确的登出调用点（例如清除 HttpOnly Cookie —— 而本阶段不下发 Cookie），
      再补一个空操作端点，或确认前端自清即可；
      ③ **首个管理员账号如何建立未决** —— 13 §5.1 的状态图起点是「管理员创建账号」，而创建端点是路线图
      （§5.3 的账号开通功能，与 D9 同批）；`Account.created_by_id` 的注释把第一个管理员记为「基建写入、无创建人」，
      但本阶段**没有任何工具**做这次基建写入。后果：空库 `init_db.py` 之后**无法登录**（无账号、无创建入口）。
      `scripts/seed_dev.py` 不预置口令（预置口令会被抄进部署脚本，而它是全系统权限的入口）——
      该理由不变，但原 docstring 写的「由 §7 的自举流程建立」在 §7 落地后已不成立，已同步订正。
      **待决**：写一个一次性脚本（交互式读口令、不落默认值）、还是把账号开通端点提前到本阶段；
...
- [ ] 9.4h **§8 暴露的标识符口径待确认项**（同样只记录、不擅自补设计）：
      ...
      本阶段在 `app/api/permissions.py` 定为 `import_template` / `cap_caliber` / `weight` / `reserve_ratio` / `prompt`
      （顺序即文档 ①~⑤；`weight` 沿用既有字面量，因为 `SUPERVISOR_CONFIG_SCOPES` 早已是它）。
      它不是内部细节而是**跨阶段契约**：阶段七的配置页按它渲染子导航胶囊、按它决定隐藏哪些子模块，
      任何一侧改名都要求另一侧同步。**待决**：是否把这五个名字写进 `22` §2.1 ⑦（推荐 —— 它已经是前后端
      共用的接口面），或确认「以代码为准、文档不必列」；
      ② **`13` 与 `22` 对配置页归属的措辞差** —— `22` §2.1 ⑦ 写「管理员专用」，而 `13` §2.2/§3.1 给主管
      🔧（仅权重）。本阶段判定两说**不冲突**（前者指页面的默认归属，后者指主管进页后被子模块范围限制），
      实现按 `13`（CLAUDE.md §八：矩阵以 `13` 为准），故**未**按勘误登记。若评审认为 `22` 的那句
      应当改为「管理员 + 主管（仅权重子模块）」，是一次文档措辞澄清，不涉及实现改动
- [x] 9.4i **§9.3 核对 #9 时发现的阶段一既有缺陷（已修）**：`evals/run_evals.py` 的
      `--compare` 帮助文本里有一个**裸百分号**（「劣化 >5% 判定失败」）。argparse 会用
      `help_string % params` 展开帮助文本（为替换 `%(default)s` 这类占位符），裸 `%` 后面跟中文
      即被当成格式说明符 → `ValueError: unsupported format character`。它发生在**构造 parser 时**，
      于是**每一次**调用（含 `--list`）都在 argparse 里崩成退出码 1 + traceback，
      从没走到文件中那段「以退出码 3 失败」的设计 —— 即 CLAUDE.md §二 的 `--list` 命令与
      §十一 的「现在跑刻意以退出码 3 失败」**两处说法都不成立**。
      该文件自阶段一首次提交（`6fdc73c`）起未改动，**非本次引入**；未被发现是因为 pre-push 的
      evals 钩子是注释状态，没有任何东西跑过它。已修（`%` → `%%`，展开后仍是 `5%`）并加
      `tests/logic/test_run_evals.py` 7 例钉住入口契约（能构造 / `5%` 字样保留 / 三层取值 /
      `--list` 返回 0 且列全 / 未实现返回 3 / `--allow-skeleton` 通过 / 失败走 stderr）。
      **待决**：无 —— 这是一处纯缺陷，修法唯一，不涉及设计口径
- [ ] 9.4j **L1 预算压力（数据 + 备选杠杆，本阶段不处置）**：9.2 实测 514 例耗时中位 ~4.28s
      （连续 5 次 4.18~4.44s），距 5 秒门禁**仅剩约 14% 余量**，且同一份代码见过一次 6.17s 的
      离群（机器噪声 —— 与杀软扫描 / 磁盘争用有关，非代码问题）。构成（单目录实测，各自含启动开销）：
      `tests/models` 196 例 2.9s（其中 **`tests/models/test_migrations.py` 9 例 1.6s，占全包约 37%** ——
      它跑真实 alembic 建库 + autogenerate，是 9.1/D8 的验收测试，**在精神上不属于 L1 单元**）、
      `tests/api` 47 例 1.2s（bcrypt 已降到 cost 4）、`tests/logic` 266 例 0.7s。
      阶段三（引擎）与阶段四（cap / 导入）都要往这个包里加测试，趋势是单向的。**备选杠杆**（按推荐序）：
      ① 把 `test_migrations.py` 移出 L1（它要真实建库，天然慢；改成 pre-push / L2 分层，或按
