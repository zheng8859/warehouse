...
系统必须在同一数据库事务内写入台账与 cap 增量（入库 ↑已占格数、出库 ↓已占格数、移库源 ↓ 目标 ↑），并整体提交或整体回滚；台账必须作为 cap 与库存分布增量的唯一来源，任何绕过台账直接改 cap 或库存分布的写入必须被拒绝。

#### Scenario: 台账与 cap 增量原子提交
- **GIVEN** 一张入库作业单确认落位
- **WHEN** 系统写入台账并更新 `AisleCap` 已占格数
- **THEN** 两者在同一事务内，要么都提交、要么都回滚

#### Scenario: 台账是增量的唯一来源
- **GIVEN** 一次落位使某巷道已占格数增加
- **WHEN** 检查该巷道 `AisleCap` 的增量来源
...
系统必须提供 `GET /api/ledger`（按作业单查询台账，含反向行 `is_reversal`）与 `GET /api/verification/{job_id}`（查询某作业单的后验结果与 `verify_result`），供验证写路径是否正确落账与后验。
