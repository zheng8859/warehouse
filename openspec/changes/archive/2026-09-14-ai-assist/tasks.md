...
- [x] 4.2 `routes/conversation.py` `POST /api/conversation/message`（L0 结构化 + L2 脱敏 LLM；写意图路由确认卡，不直接写台账）。验证：API 契约单测——写意图返回建议 + `write_intent=true` 且零台账。
...
- [x] 5.3 `deps.py` / 端点接入 `require_permission`（`ai.assist` / `ai.weight.update` / `ai.relocate.propose` / `ai.toggle`）。验证：403 单测——计划员调 `ai.assist` 403、非管理员调 toggle 403。

## 6. 异常/边界与回归

- [x] 6.1 边界测试：开关关闭 → 409 `cold_path_disabled`；LLM 失败 → 200 降级（不 5xx）；月度预算耗尽 → 熔断仅规则卡片；权限越界 → 403。验证：`tests/api/` 边界套件全绿。
- [x] 6.2 全量回归：`python -m pytest tests/ --tb=short -q` 确认核心链路（评分/落位/后验/台账）1166 条既有测试零回归，冷路径新增测试全绿。
