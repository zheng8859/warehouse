...
- [x] 5.2 用真实数据冒烟：对 `BI看板需求 -GTJ10036.xlsx` 跑脚本，断言 A/B/C 料号数 = 20/31/204（对齐 `16` A.4.1 实测）；验证输出一致
...
- [x] 7.1 覆盖异常路径：时点缺失/未来时点阻断、字段未 100% 命中阻断、格式/编码不可识别阻断、快照缺失阻断（不猜测落位）、乐观锁并发冲突；对应 `tests/logic` 与 `tests/api` 负例，验证 `pytest tests/ --tb=short -q` 全绿
- [x] 7.2 端到端集成冒烟：上传三类文件 → 校验 → 执行导入 → 快照/队列/任务/cap 基线落库 → 重复导入幂等；验证 `tests/api/test_import_e2e.py` 通过
