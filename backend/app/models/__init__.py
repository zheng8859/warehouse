"""SQLAlchemy 模型 —— 26 个实体，按 17 号四条数据链 + 冷路径链分组。

事实来源：17-数据模型设计（实体共 23 个，枚举 11 个）、10 号 / 29 号（冷路径 3 个）
  ...
  冷路径链  AiSuggestion / ConversationLog / AiCostQuota       llm.py              (3)

  合计 6 + 5 + 5 + 1 + 1 + 5 + 3 = 26

注意：`操作与确认记录`（17 §4.5）不是实体之一 —— 它没有英文实体名、不在 ER 图中，
处置结果以 disposition 枚举落在 JobOrder 上。不要为它新建模型。冷路径链 3 实体
**不进 `Ledger` 台账链**（spec `data-model`「冷路径实体非台账」），不要把它们挂到
作业链下。
